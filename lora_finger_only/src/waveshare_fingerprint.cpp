...
waveshare_fingerprint::EFPErrors waveshare_fingerprint::enroll_fingerprint(uint16_t slot, uint8_t access_level, waveshare_fingerprint::EEnrollStage stage)
{
    ...
    if (!read_success) {
        Serial.println("Failed to read response from sensor");
        return ACK_FAIL;
    }
    
    Serial.print("Enrollment response: ");
    for (int i = 0; i < 8; i++) {
...
waveshare_fingerprint::EFPErrors waveshare_fingerprint::check_user_exist(uint16_t slot)
{
    uint8_t response[8] = {0};

    send_command(CMD_CHECK_USER_EXIST, (slot >> 8) & 0xff, slot & 0xff, 0);
    return read_response(response, sizeof(response)) ? (waveshare_fingerprint::EFPErrors) response[4] : ACK_FAIL;
}
